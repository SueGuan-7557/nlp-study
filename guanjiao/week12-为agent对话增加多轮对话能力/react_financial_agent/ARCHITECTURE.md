...
用户问题
    ...
    ▼
┌─────────────────────────────────────────────────────┐
│ 会话层：session_id → 最近 N 轮 user/assistant 消息  │
└──────────────────────────┬──────────────────────────┘
                           ▼
┌─────────────────────────────────────────────────────┐
│               ReAct 循环（最多 10 步）               │
...
- Web UI 为每段对话生成并复用 `session_id`，后续问题天然成为追问。
- `serve.py` 中的 `ConversationStore` 保存用户消息和最终回答，不保存 Thought、工具调用及 Observation，避免内部轨迹污染后续上下文。
- 同一会话使用异步锁串行执行，防止并发追问覆盖历史。
- 仅在 Agent 成功产出 `final` 时提交本轮历史；失败或达到最大步数的轮次不会写入。
- 默认每个会话保留最近 10 轮，并按 LRU 限制为 200 个会话，防止内存无界增长。
- 两种 ReAct 实现都通过 `history` 参数接收统一的 `user`/`assistant` 消息格式。
